...
class Encoder(nn.Module):
    ...
    def forward(self, sym, p_sym):
        constellation = torch.from_numpy(tu.qamConst(chParam.M)).to(self.device)
        constellation = constellation.flatten()

        norm_factor = torch.rsqrt(self.p_norm(p_sym, constellation))
        norm_constellation = norm_factor * constellation
        norm_constellation = norm_constellation.unsqueeze(1)

        norm_constellation_real = tu.complex2real(norm_constellation)
        enc_real = torch.matmul(sym, norm_constellation_real)
        enc = torch.complex(enc_real[:, 0], enc_real[:, 1]).unsqueeze(1)

        enc = tu.complex2real(enc)
        norm_constellation = tu.complex2real(norm_constellation)

        return enc, norm_constellation


encoder = Encoder(num_gpu=trainingParam.gpuNum, device=device)      # encoder object
...
print('\nInitializing the Decoder ......')    # to mean=0, stdev=0.02
...
class CELossForPCS(nn.Module):
    ...
    def entropy_S(self, p_sym):
        return -(torch.sum(p_sym * torch.log2(p_sym)))
...
with torch.no_grad():
    ...
    plt.savefig('Constellation after training.jpg')
    plt.show()
    ...
    plt.savefig('Possibility after training.jpg')
    plt.show()
...
with torch.no_grad():
    ...
    checkpoint = torch.load(trainingParam.path)
    pcs_nn.load_state_dict(checkpoint['pcs_nn_state_dict'])
