...
import numpy as np
# import cupy as np
# print('cupy version: {}'.format(np.__version__))
import tools.utils as cu
...
def defaultParameters(D=16.4640, Fc=1.9341e+14):
    ...
    param.PdBm = 1
    param.Rs = 32 # e9      symbol rate [GBaud]
...
def normalizeParameters(param):
    ...
    param.T  = 1000/param.Rs                            # symbol duration [ps]
    param.P0 = cu.dB2lin( param.PdBm, 'dBm' )           # launch power
...
def calcInterChannelNLIN(chi,kur,param):
    param = normalizeParameters(param)
    ...
    NLIN_inter = chi[0,:]+(kur-2)*chi[1,:]
    if param.nPol == 2:
        # NLIN_inter = 16/81*(NLIN_inter+2*chi[0,:]/4+(param.kur-2)*chi[1,:]/4)
        NLIN_inter = 16/81*(NLIN_inter+2*chi[0,:]/4+(kur-2)*chi[1,:]/4)
    NLIN_inter = param.P0**3*NLIN_inter

    return NLIN_inter

# def calcInterChannelNLINAddTerms(X,param):
def calcInterChannelNLINAddTerms(X,kur,param):
    param = normalizeParameters(param)

    if X.ndim==1:
        X = np.expand_dims(X,1)

    NLIN_inter_addTerms = 4*X[0,:]+4*(kur-2)*X[1,:]+2*X[2,:]+(kur-2)*X[3,:]
    if param.nPol == 2:
        NLIN_inter_addTerms = 16/81*(NLIN_inter_addTerms+2*X[0,:]+(kur-2)*X[1,:]+X[2,:]+0*(kur-2)*X[3,:])
    NLIN_inter_addTerms = param.P0**3*NLIN_inter_addTerms
    return NLIN_inter_addTerms

# def calcIntraChannelNLIN(X,param):
def calcIntraChannelNLIN(X,kur, kur3, param):
    param = normalizeParameters(param)
    ...
    NLIN_intra = 2*X[1,:]+(kur-2)*(4*X[2,:]+X[3,:])+(kur3-9*kur+12)*X[4,:]-(kur-2)**2*X[0,:]
    if param.nPol == 2:
        # NLIN_intra = 16/81*(NLIN_intra+X[1,:]+(param.kur-2)*X[2,:])
        NLIN_intra = 16/81*(NLIN_intra+X[1,:]+(kur-2)*X[2,:])
